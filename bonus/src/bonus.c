...
/*   Updated: 2021/04/28 16:39:38 by melperri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		render_life(t_all *all)
{
	...
	if (all->win_ptr == NULL)
		return (1);
	drawstart = all->rx / 2 - (all->rx / 2 - all->rx / 3);
	drawend = (all->rx / 2) + (all->rx / 2 - all->rx / 3);
	red_drawend = ((all->rx / 2 + (all->rx / 2 - all->rx / 3)) * all->player.hp) / 100;
	y = 10;
	x = drawstart - 1;
	while (x < drawend + 1)
